...
Aeneas rejects `union` types, so the union-based `BinderObject` deserializer did
not reach Lean (see [AENEAS-REPORT.md](AENEAS-REPORT.md)). This stage re-models
it as a byte array with typed accessors, the design agreed with the Aeneas
authors in [AeneasVerif/aeneas#1199](https://github.com/AeneasVerif/aeneas/issues/1199),
so the parse path (`read_from`, `read_from_inner`, accessors, `parse_one`)
translates to Lean. The goal is the maintainer's property: for all byte inputs
from userspace, the code does not panic.

The remodel ([remodel/](remodel/)) translates through Charon and Aeneas and
proves the target. `parse_one_no_panic` states that for all byte inputs
`parse_one` returns `ok`, never `fail`; it is machine-checked and `sorryAx`-free,
modulo the opaque `size_of` axiom. No-panic is also proved for every accessor,
the reader primitives (`read_slice`, `skip`, `len`), `size`/`type_to_size`, and
the three validators. The `lean/` project builds clean.
...
non-advancing clone, then `skip`s the object's size. This matches the kernel
algorithm byte-for-byte. `cargo test`: 7/7 (round-trips per variant, EINVAL
rejection, `type_to_size`/validator checks; the offset/size assertions are
compile-time).
...
2. Control flow is preserved. `read_from`/`read_from_inner`/`parse_one` mirror
   `allocation.rs` step for step: same `min`/`clone_reader`/`read_slice`/`skip`
   protocol, same tag check → `EINVAL`, same `reader.skip(size)`.
3. Every deviation is marked `// REMODEL:` in [remodel/src/lib.rs](remodel/src/lib.rs).
   The material ones:

   | Kernel | Remodel | Why |
   |---|---|---|
   | `union BinderObject` | `[u8; 40]` | Aeneas has no unions |
...
accessors, `size`, `parse_one`. Two changes to the crate were needed, both marked
`REMODEL`:

- Generic `FnOnce` over `&mut [u8; N]`. Aeneas errored (`expected a product type`
  / `Type mismatch` at the closure's `call_once`): it does not thread the
  mutable-array write-back through a generic `FnOnce` instance, since the trait's
  `call_once` return type drops the mutated buffer. The closure now takes the
  buffer by value and returns it filled, with the same control flow.
- Anonymous `const _` layout assertions. Aeneas emitted an invalid Lean `def _`
  for them. They move into `#[cfg(test)]` (Charon builds without `--test`), so
  they are checked at compile time but not extracted.

Translation is then clean, with only the advisory `-split-files` warning.
...
`Array.index_usize` reads (fallible: out-of-bounds ⇒ `fail`) plus the total
...
The panic model is described in [AENEAS-REPORT.md](AENEAS-REPORT.md): a panic is
the `Result` monad's `fail`, and no-panic is `∀ inputs, ∃ v, f inputs = ok v`. In
the remodel the fail sources are array indexing (`arrayOutOfBounds`), the `usize`
subtraction in `SliceReader.len` (`data.len - pos`, underflow), and the opaque
`core.mem.size_of` axiom (via `type_to_size`); `from_le_bytes` is total.
`parse_one` builds every reader with `pos = 0` and does not advance the original
...
`#print axioms` confirms every "proved" theorem below is `sorryAx`-free (standard
`propext`/`Classical.choice`/`Quot.sound`, plus `core.mem.size_of` for the
size-dependent ones, the same opaque-`size_of` assumption noted in AENEAS-REPORT).
...
| `is_aligned_no_panic` (+ `_panics_on_zero`) | no-panic iff `to ≠ 0`; panics at `to = 0` | **Proved** |
...
uses, on which every op is unconditionally total. Only `size_of`-dependent
theorems carry the opaque-`size_of` assumption (see AENEAS-REPORT); `parse_one`
inherits it via `type_to_size`. `ptr_align` was not attempted here; it remains
`sorry` in `NoPanic.lean` with the Try-trait note from the previous stage.

---

## 6. Proving `parse_one_no_panic`

The proof runs `parse_one` by symbolic execution (`step*`) over the sub-specs,
then case-splits the `?`/`branch` control flow; every arm ends in `ok (…)`, so
the only remaining assumption is the opaque `size_of`.

One obstruction was tactical. `step`'s output-destructuring (`introOutputs`)
fires only for spec applications with no side goal, and the earlier reader specs
carried a `pos ≤ data.len` precondition, so `step` left each tuple result as an
unreducible pattern-`let`. Since `parse_one` uses only `pos = 0` readers, on which
`len` cannot underflow and `read_slice`/`skip` return an `Err` value, the reader
specs were restated precondition-free for `⟨data, 0#usize⟩`; `step*` then
destructures them, as in Aeneas's tuple-bind tests
(`Tactic/Step/Tests/UncurryBind.lean`). Two results whose specs still carry side
goals (`index_mut`, `read_slice`) are destructured with one `obtain` each; the
branch cascade closes with `simp only [] at *` (matcher-on-constructor iota),
`step*`, and `casesm`/`split`.
